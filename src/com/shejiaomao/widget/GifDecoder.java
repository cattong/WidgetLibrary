...
public class GifDecoder extends Thread {
	...
	private int decodeGif() {
		...
		try {
		    ...
		    if (WidgetConstants.isDebug) Log.d(TAG, TAG, e);
		}
	...
	private GifFrame createFrame(GifImageData imgData, int frameCount, int delay) {
		...
		if (WidgetConstants.isDebug) MemoryManager.trace();
	...
	private void fillFramePixels(GifFrame frame) {
        ...
        if (WidgetConstants.isDebug) Log.d(TAG, "graphic control disposal:" + gcExt.disposal);
		int tempColor = 0;
		if (gcExt.transparentColorFlag) {
