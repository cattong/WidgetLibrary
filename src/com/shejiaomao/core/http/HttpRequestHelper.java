...
import com.shejiaomao.core.http.config.HttpConfig;
import com.shejiaomao.core.http.config.HttpConfigFactory;
...
public class HttpRequestHelper {
	...
	private static synchronized ClientConnectionManager createConnectionManager() {
		...
		//设置连接池允许的最大连接数
		ConnManagerParams.setMaxTotalConnections(managerParams, LibConstants.CONNECTION_POOL_SIZE);
		//设置连接池连接超时时间
		ConnManagerParams.setTimeout(managerParams, LibConstants.CONNECTION_TIME_OUT);
	...
	public static <T> T execute(HttpRequestWrapper httpRequestWrapper,
		...
		try {
			...
			if (LibConstants.level <= LibConstants.VERBOSE) {
				...
				logger.trace("HttpRequestMessage {}:{}", httpRequestWrapper.getMethod(), httpRequestWrapper.getUrl());
				logger.trace("HttpRequestMessage params:{}", httpRequestWrapper.getParameters());
			}
	...
	public static byte[] getContentBytes(String url) throws LibException {
		if (LibConstants.isDebug()) logger.trace("Download url content : {} " , url);
		byte[] byteArray = null;
	...
	public static File getBitmapFile(String url, File destFile) throws LibException {
		if (LibConstants.isDebug()) logger.trace("Download Image : {} " , url);
	...
	public static void evictConnections() {
		...
		synchronized (HttpRequestHelper.class) {
			if (LibConstants.isDebug()) logger.trace("Connections Eviction");
